...
CUMCM_NO_AI_FILENAME = "AI工具使用声明.md"
...
def fill_template_main(
    ...
    # 复制其他 sty / cls 文件
    common_dir = template_dir.parent / "common"
    if common_dir.exists():
        for f in common_dir.glob("*.sty"):
...
def compile_pdf(tex_path: Path, engine: str = "xelatex", runs: int = 3, timeout: int = 180) -> bool:
    try:
    ...
    if is_tectonic:
        runs = 1  # Tectonic manages reruns and bibliography itself.
    for i in range(runs):
        print(f"\n--- {engine} 第 {i+1}/{runs} 次 ---")
        if is_tectonic:
            out_arg, tex_arg = str(workdir), str(tex_path.resolve())
            if os.name != "nt" and engine.lower().endswith(".exe") and shutil.which("wslpath"):
                out_arg = subprocess.check_output(["wslpath", "-w", out_arg], text=True).strip()
                tex_arg = subprocess.check_output(["wslpath", "-w", tex_arg], text=True).strip()
            command = [engine, "--print", "--keep-logs", "--keep-intermediates", "--outdir", out_arg, tex_arg]
        else:
            command = [engine, "-interaction=nonstopmode", "-halt-on-error", "-no-shell-escape", str(tex_path.name)]
        try:
            result = subprocess.run(command, cwd=workdir, capture_output=True, text=True,
                                    encoding="utf-8", errors="replace", timeout=timeout)
        except (OSError, subprocess.TimeoutExpired) as exc:
            print(f"[FAIL] compiler unavailable or timed out: {exc}")
            if isinstance(exc, subprocess.TimeoutExpired):
                partial = exc.stdout or b""
                print(partial.decode("utf-8", errors="replace")[-2000:] if isinstance(partial, bytes) else partial[-2000:])
            return False
...
def main():
    ...
    parser.add_argument("--engine", help="Override compiler: xelatex, pdflatex, tectonic or executable path")
    parser.add_argument("--compile-timeout", type=int, default=180, help="Seconds per compiler invocation")
    ...
    if not args.no_compile:
        try:
            from render_ai_usage import render_reports
            render_reports(decision_log_path, workspace, workspace.resolve().parent / "support_materials", competition)
        except (ValueError, OSError, RuntimeError) as exc:
            print(f"[FAIL] AI disclosure generation: {exc}")
            return 1
    ...
    return 0 if compile_pdf(tex_path, engine=args.engine or engine, runs=3, timeout=args.compile_timeout) else 1
