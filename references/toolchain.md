...
求解代码内部使用 workspace-relative path、CLI/config/manifest 定位真实文件。命令行本身当然可以传绝对的 `<project>` 入口；运行日志也可记录解析后的绝对路径，但交付代码不得把个人机器路径写死。
...
```bash
python <skill>/scripts/check_layout.py <project>/paper_workspace/main.pdf \
  --tex <project>/paper_workspace/main.tex \
  --output <project>/state/layout-lint.json
```
...
规范见 `references/architecture-diagram.md`：
