...
def run_checks(
    ...
    required_paths = (
        ...
        "templates/latex/common/pandoc-compat.sty",
    )
    ...
    checks.append(_check(
        "skill-metadata",
        skill_name == "cumcm-team-ai",
        f"root={skill_name!r}; project-local skill distribution",
    ))
    ...
    if check_tools:
        engine = engine_override or RENDER_ENGINES[competition]
        ...
        required_tex_files = () if "tectonic" in Path(engine).name.lower() else REQUIRED_TEX_FILES[competition]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check cumcm-team-ai readiness.")
    parser.add_argument("--competition", choices=COMPETITIONS, default="cumcm")
    parser.add_argument("--workspace", type=Path, default=None)
    parser.add_argument("--engine", help="Override renderer engine, including a Tectonic executable path")
