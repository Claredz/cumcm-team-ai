...
Outputs are placed where the paper renderer expects them:
  ...
  * cumcm (used or not): ``paper_workspace/AI工具使用声明.md`` before references
...
CUMCM_NO_USE_FILENAME = "AI工具使用声明.md"
...
def resolve_competition(cli_arg: Optional[str], decision_log: Dict[str, Any]) -> str:
    ...
    if competition not in {"cumcm", "mcm", "diangong"}:
        raise LedgerValidationError(
...
def render_cumcm_no_use_statement() -> str:
    """CUMCM 2026 declaration before references."""
...
def render_cumcm_pdf(
    ...
    """Render an A4 Chinese disclosure PDF using a CJK CID font."""
    if not entries:
        raise LedgerValidationError(
...
def render_reports(
    ...
    *,
    ...
    if selected == "diangong":
        support_dir.mkdir(parents=True, exist_ok=True)
        output = support_dir / "AI使用台账-待核对当届提交要求.md"
        output.write_text("# 内部 AI 使用台账\n\n当届专门格式未核实，不能视为官方提交声明。\n\n" +
                          (render_cumcm_markdown(entries, decision_log) if entries else
                           "团队已明确记录未使用 AI。\n"), encoding="utf-8")
        return [output]
    if selected == "mcm":
        if markdown_only:
    ...
    if not entries:
        (paper_workspace / "AI工具未使用声明.md").unlink(missing_ok=True)
        for stale_path in (
    ...
    paper_workspace.mkdir(parents=True, exist_ok=True)
    (paper_workspace / "AI工具未使用声明.md").unlink(missing_ok=True)
    purposes = "、".join(dict.fromkeys(entry.purpose for entry in entries))
    (paper_workspace / CUMCM_NO_USE_FILENAME).write_text(
        f"本参赛队在竞赛过程中使用了AI工具，主要用于{purposes}，详细使用情况见支撑材料。\n", encoding="utf-8")
