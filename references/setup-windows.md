...
本页面向不使用 WSL 的队员。目标是让 Windows PowerShell 成为一等支持环境，而不是要求队员比赛前临时学习 Linux。
...
如果正式论文用 XeLaTeX，再执行一次真实短稿编译和 `pdf_audit.py`。不要在比赛开始后才首次安装 TeX 或修 PATH。
